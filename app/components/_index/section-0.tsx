...
export default function Index() {
  ...
  const createShopifyOrder = async (): Promise<boolean> => {
    try {
      const res = await fetch("api/order/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ items, totals, data, invoice_id }),
      });
      const result = await res.json();

      if (!res.ok || !result.success) {
        console.error("Shopify order creation failed:", result.errors);
        return false;
      }
      return true;
    } catch (err) {
  ...
  const handlePdfButton = async () => {
    const orderCreated = await createShopifyOrder();
    if (!orderCreated) {
      return;
    }
  ...
  const generateInvoicePdf = async (): Promise<number> => {
    ...
    if (hasCustomerUID && taxZero) {
      ...
      doc.text('Steuerfreie innergemeinschaftliche Lieferung gemäß § 7 UstG i. V.m.Art 138 MwStSystRL.', 40, ty + 26);
      doc.setTextColor(0);
    }
    ...
    if (note) {
      doc.setFont('helvetica', 'bold');
      doc.text('Hinweis', 40, ty + 42);
      doc.setFont('helvetica', 'normal');
      doc.text(doc.splitTextToSize(note, 480), 40, ty + 58);
    }
  ...
  return (
  <s-stack gap="base">
      <s-banner heading="Feature Notice" tone="info">
        Automatic Order creation is still in beta. Expect bugs.
      </s-banner>
    <s-box
      padding="base"
      background="base"
